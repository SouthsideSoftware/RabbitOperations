...
namespace RabbitOperations.Collector.Tests.Unit.Service
{
    ...
    public class ActiveQueuePollersTests
    {
        ...
        public void ShouldNotDoAnythingWhenTryingToRemoveAPollerThatDoesNotExist()
        {
            ...
            var queuePoller2 = new ApplicationListener(key2, new QueueSettings("audit", new ApplicationConfiguration
            {
                ApplicationId = "test"
            }));
            ...
            //assert 
            activeQueuePollers.ActivePollers.Should().BeEquivalentTo(new List<IApplicationListener> {queuePoller});
        }
